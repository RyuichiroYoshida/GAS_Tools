const routes = {
	releases: "Releasesの一覧取得",
	asset: "アセットのURL取得 parameter:id",
	default: "ルート一覧",
};
 ...
 *
 * @param {Object} e - イベントパラメータ（リクエスト情報を含む）。
...
function doGet(e) {
	...
	if (response) {
		...
		return ContentService.createTextOutput(JSON.stringify({ error: "Invalid route" })).setMimeType(ContentService.MimeType.JSON);
	}
}
